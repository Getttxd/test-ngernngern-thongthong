...
export function createTransactionRouter() {
  ...
  router.get(
    ...
    describeRoute({
      tags: ['Transactions'],
      summary: 'List transactions by type (income/expense)',
      responses: {
        200: { description: 'Filtered transactions', content: jsonContent(transactionListResponseSchema) },
      },
    }),
    validator('param', typeParamSchema),
    (c) => c.get('container').transactionHandler.listByType(c)
  )
  ...
  router.get(
    ...
    (c) => c.get('container').transactionHandler.listByDateRange(c)
  )
